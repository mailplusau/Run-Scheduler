...
$(document).on('change', '.customer_row', function(e) {
	if ($('option:selected', this).val() > 1) {
		...
		// }
		console.log('inside create new ncl')
		var upload_url = baseURL + nlapiResolveURL('SUITELET', 'customscript_sl_setup_stops', 'customdeploy_sl_setup_stops') + '&custparam_params=' + params;
		window.open(upload_url, "_self", "height=750,width=650,modal=yes,alwaysRaised=yes");
	}
});

$(document).on('change', '.customer_selected_class', function(e) {
	var zee = parseInt(nlapiGetFieldValue('custpage_zee'));
	...
	if ($('option:selected', this).val() != 0) {
		...
		resultSetCustomer.forEachResult(function(searchResult_service) {
			var service_id = searchResult_service.getValue("internalid", null, "GROUP");
			var service_name = searchResult_service.getText('custrecord_service', null, "GROUP");
			var service_leg_freq_count = searchResult_service.getValue("internalid", "CUSTRECORD_SERVICE_FREQ_SERVICE", "COUNT");
			var service_leg_count = searchResult_service.getValue("internalid", "CUSTRECORD_SERVICE_LEG_SERVICE", "COUNT");
			var service_price = searchResult_service.getValue("custrecord_service_price", null, "GROUP");
...
$(document).on('change', '.service_selected_class', function(e) {
	if ($('option:selected', this).val() != 0) {
		...
		var mon = service_record.getFieldValue('custrecord_service_day_mon');
		var tue = service_record.getFieldValue('custrecord_service_day_tue');
		var wed = service_record.getFieldValue('custrecord_service_day_wed');
		var thu = service_record.getFieldValue('custrecord_service_day_thu');
		var fri = service_record.getFieldValue('custrecord_service_day_fri');
		var adhoc = service_record.getFieldValue('custrecord_service_day_adhoc');
